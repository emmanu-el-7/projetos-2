...
const getAllPhoto = async (req, res) => {
  //a função find executa um filtro; se levar um parãmetro vazio, busca todos
...
const getPhotoById = async (req, res) => {
  ...
  try {
    const photo = await Photo.findById(new moongose.Ttypes.ObjectId(id))
...
const updatePhoto = async (req, res) => {
  ...
  try {
    const { title } = req.body //só podemos atualizar o título da foto

    const reqUser = req.user

    const photo = await Photo.findById(id)

    //caso não exsista, envia mensagem de erro
    if (!photo) {
      res.status(404).json({ errors: ['Foto não encontrada.'] })
      return
    }

    //verifica se a foto pertence ao usuário, pois apenas o dono pode alterá-la
    if (!photo.userId.equals(reqUser._id)) {
      res
        .status(422)
        .json({ errors: ['Ocorreu um erro, tente novamente mais tarde'] }) //erro genérico
